...
// Latihan Unit Testing GetProducts
func TestProductController_GetProducts_Success(t *testing.T) {
	...
	var resBody []product_domain.Product
	err = json.Unmarshal(data, &resBody)

	assert.Nil(t, err)
	assert.NotNil(t, resBody)

	assert.EqualValues(t, resBody[0].Id, product.Id)
	assert.EqualValues(t, resBody[0].Name, product.Name)
	assert.EqualValues(t, resBody[0].Price, product.Price)
	assert.EqualValues(t, resBody[0].Stock, product.Stock)

	assert.EqualValues(t, resBody[1].Id, product.Id)
	assert.EqualValues(t, resBody[1].Name, product.Name)
	assert.EqualValues(t, resBody[1].Price, product.Price)
	assert.EqualValues(t, resBody[1].Stock, product.Stock)
}
